...
int MotorCorrection = 0.885;		// Percent to decrease left motors
...
void mvtForwardLeft(signed char speed) {
  ...
  if (!isFlipped) {
    motor[leftMaster] = -speed * MotorCorrection;
  } else {
...
task autonomous() {
	...
	*/
	...
	writeDebugStreamLine("Right Movement:");
	writeDebugStreamLine("> Left Encoder %i", SensorValue(leftEncoder));
	writeDebugStreamLine("> Right Encoder %i", SensorValue(rightEncoder));
